...
use crate::{value, KvError, Kvpair, Storage, StorageIter, Value};
...
impl TryFrom<Value> for Vec<u8> {
    ...
    fn try_from(v: Value) -> Result<Vec<u8>, Self::Error> {
        if let Some(value::Value::String(s)) = v.value {
            return Ok(s.into());
        }
        return Err(KvError::Internal("unsupport type".to_string()));
    }
}

impl From<sled::Error> for KvError {
    fn from(e: sled::Error) -> Self {
        KvError::Internal(format!("{}", e))
    }
}
...
impl Storage for SledDb {
    fn get(&self, table: &str, key: &str) -> Result<Option<Value>, KvError> {
        ...
        let result = self.0.get(name.as_bytes())?.map(|v| Ok(Value::from(v)));
        flip(result)
    }
    ...
    fn set(&self, table: &str, key: String, value: Value) -> Result<Option<Value>, KvError> {
        ...
        let result = self.0.insert(name, data)?.map(|v| Ok(Value::from(v)));
        flip(result)
    }
    ...
    fn del(&self, table: &str, key: &str) -> Result<Option<Value>, KvError> {
        ...
        let result = self.0.remove(name)?.map(|v| Ok(Value::from(v)));
        flip(result)
    }
